...
import { renderFrozenAnalysisPoolCompletionMarkdown } from '../../modules/analysis/helpers/frozen-analysis-pool.helper';
...
async function main() {
  ...
  try {
    ...
    const report = await service.buildFrozenAnalysisPoolCompletionReport({
      refreshSnapshot: options.refresh === true,
    });
    const markdown = renderFrozenAnalysisPoolCompletionMarkdown(report);
    ...
    const markdownPath = path.join(
      outputDir,
      `frozen-analysis-pool-completion-${stamp}.md`,
    );
    const jsonPath = path.join(
      outputDir,
      `frozen-analysis-pool-completion-${stamp}.json`,
    );
    ...
    const payload = options.json
      ...
      : {
          ...
          completionSummary: {
            completedUseful: report.frozenPoolCompletedUsefulCount,
            completedDeleted: report.frozenPoolCompletedDeletedCount,
            completedArchived: report.frozenPoolCompletedArchivedCount,
            remaining: report.frozenPoolRemainingCount,
