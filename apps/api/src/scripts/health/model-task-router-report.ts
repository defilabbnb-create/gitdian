...
} from '../../modules/analysis/helpers/model-task-router.helper';
import {
...
} from '../../modules/analysis/helpers/model-task-router-summary.helper';
...
async function main() {
  ...
  try {
    const inventoryReport = buildModelTaskRouterInventoryReport();
    ...
    const decisionReport = buildModelTaskRouterDecisionReport({
      priorityGeneratedAt: priorityReport.generatedAt,
      repairItems: priorityReport.items,
    });
    const executionReport = buildModelTaskRouterExecutionReport({
      priorityReport,
      queueSummary,
      healthReport:
    ...
    const markdownPath = path.join(outputDir, `model-task-router-${stamp}.md`);
    const jsonPath = path.join(outputDir, `model-task-router-${stamp}.json`);
