...
export function buildIdeaFitPromptInput(repository: RepositoryWithContext) {
  const fileTreeEntries = normalizeJsonArray(repository.content?.fileTree).filter(
    (item) => !!item && typeof item === 'object' && !Array.isArray(item),
  ) as Array<Record<string, unknown>>;

  const fileTree = prioritizeFileTreeEntries(fileTreeEntries).slice(0, 160);
  ...
  return {
    ...
    content: {
      readmeText: truncateText(repository.content?.readmeText, 7000),
