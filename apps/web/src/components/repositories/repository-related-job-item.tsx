  ...
  return (
    <article className="rounded-[28px] border border-slate-200 bg-slate-50 p-5">
      ...
      <div className="mt-4 grid gap-3 xl:grid-cols-2">
        <SummaryCard title="Payload 摘要" text={summarizeObject(job.payload)} />
        <SummaryCard title="Result 摘要" text={summarizeObject(job.result)} />
      </div>
      ...
      <details className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-3">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          <JsonPanel title="Payload JSON" value={job.payload ?? null} />
          <JsonPanel title="Result JSON" value={job.result ?? null} />
        </div>
      </details>
    </article>
  );
